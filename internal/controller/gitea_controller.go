...
func (r *GiteaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Client.Get(ctx, req.NamespacedName, gitea); err != nil {
	...
	if err := r.Client.Get(ctx, nns, secret); err != nil {
		if errors.IsNotFound(err) {
			r.Recorder.Eventf(
				gitea,
				secret,
	...
	meta.SetStatusCondition(&gitea.Status.Conditions, v1.Condition{
		...
		Message: fmt.Sprintf("Succesfully connected to Gitea instance '%s' in namespace '%s'.", gitea.Name, gitea.Namespace),
	})

	err = r.Client.Status().Update(ctx, gitea)
	if err != nil {
