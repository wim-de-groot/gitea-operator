...
func (r *GiteaRepoReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, giteaNamespacedName, gitea); err != nil {
		if errors.IsNotFound(err) {
			r.Recorder.Eventf(
				repo,
				gitea,
				...
				"Degraded",
				"GetGiteaInstance",
				"Gitea instance '%s' not found in namespace '%s'",
				repo.Spec.GiteaRef.Name,
				repo.Spec.GiteaRef.Namespace,
			)
	...
	if err := r.Get(ctx, types.NamespacedName{Name: gitea.Spec.CredentialsSecretName, Namespace: gitea.Namespace}, secret); err != nil {
		if errors.IsNotFound(err) {
			r.Recorder.Eventf(
				gitea,
				secret,
	...
	if err != nil {
		r.Recorder.Eventf(
			...
			"Degraded",
			"ConnectionToGitea",
			"Credentials '%s' for Gitea instance '%s' not correct, because of '%s'",
			gitea.Spec.CredentialsSecretName,
			gitea.Name,
			err,
		)
