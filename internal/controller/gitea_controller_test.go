...
var _ = Describe("Gitea Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kid Gitea")
			err := k8sClient.Get(ctx, typeNamespacedName, gitea)
			if err != nil && errors.IsNotFound(err) {
				resource := &corealphav1.Gitea{
					ObjectMeta: metav1.ObjectMeta{
						Name:      resourceName,
						Namespace: "default",
					},
					Spec: corealphav1.GiteaSpec{
						Url:                   "https://gitea.united",
						CredentialsSecretName: "gitea-initial-admin-credentials",
		...
		It("should successfully reconcile the resource", func() {
			...
			err = k8sClient.Get(ctx, typeNamespacedName, resource)
			Expect(err).NotTo(HaveOccurred())
			Expect(resource.Status.Connected).Should(BeTrue())
		})
	})
})
