...
var _ = Describe("GiteaRepo Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			...
			err := k8sClient.Get(ctx, typeNamespacedName, gitearepo)
			if err != nil && errors.IsNotFound(err) {
				resource := &corealphav1.GiteaRepo{
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance GiteaRepo")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &GiteaRepoReconciler{
				Client: k8sClient,
